...
async fn test_create_workflow_task_edge_success() {
    ...
    let test_workflow_task2 = common::workflow_task::test_workflow_task(
        &db,
        test_workflow.id.to_string().clone(),
...
async fn test_delete_workflow_task_edge_success() {
    ...
    let wt1 = common::workflow_task::test_workflow_task(&db, workflow.id.to_string(), task1.id.to_string(), 1).await;
    let wt2 = common::workflow_task::test_workflow_task_dyn(&db, workflow.id.to_string(), task2.id.to_string(), 2, "Step_2".into()).await;
    let edge = common::worflow_task_edge::test_workflow_task_edge(&db, workflow.id.to_string(), wt1.id.to_string(), wt2.id.to_string()).await;
    let app = common::setup_app(db).await;
    let resp = test::call_service(
        &app,
...
async fn test_workflow_task_edge_endpoints_unauthenticated() {
    ...
    common::truncate_db(&db).await;
    let app = common::setup_app(db).await;
    let create_url = common::path::Paths::CREATE_WORKFLOW_TASK_EDGE.replace("{workflow_id}", NONEXISTENT_ID);
    ...
    for resp in [
        test::call_service(&app, test::TestRequest::post().uri(&create_url).set_json(&json!({})).to_request()).await,
...
async fn test_create_workflow_task_edge_duplicate() {
    ...
    let wt1 = common::workflow_task::test_workflow_task(&db, workflow.id.to_string(), task1.id.to_string(), 1).await;
    let wt2 = common::workflow_task::test_workflow_task_dyn(&db, workflow.id.to_string(), task2.id.to_string(), 2, "Step_2".into()).await;
    common::worflow_task_edge::test_workflow_task_edge(&db, workflow.id.to_string(), wt1.id.to_string(), wt2.id.to_string()).await;
    let app = common::setup_app(db).await;
    let resp = test::call_service(
        &app,
        test::TestRequest::post()
            .uri(&common::path::Paths::CREATE_WORKFLOW_TASK_EDGE.replace("{workflow_id}", &workflow.id.to_string()))
            .set_json(&json!({
