...
use crate::db::entity::workflow::Model as WorkflowRow;
...
use crate::dto::workflow_task::WorkflowTaskResponse;
    ...
    fn validate(&self) -> Result<(), ValidationError> {
        ...
        if self.run_type == WorkflowRunType::Scheduled.to_string() {
            if let Some(ref schedule_time) = self.schedule_time {
                if chrono::DateTime::parse_from_rfc3339(schedule_time).is_err() {
                    return Err(ValidationError {
                        field: String::from("schedule_time"),
